...
EBTNodeResult::Type UBTTask_SpawnSwamp::ExecuteTask(UBehaviorTreeComponent& OwnerComp, uint8* NodeMemory)
{
	...
	FGameplayAbilitySpec* SkillGASpec = Monster->GetAbilitySystemComponent()->FindAbilitySpecFromClass(USZGA_BansheeSwampSpawn::StaticClass());
...
void UBTTask_SpawnSwamp::OnSkillFinished(bool bSuccess)
{
	if (!CachedOwnerComp)
	{
		return ;
