...
UBTTask_StormAttack::UBTTask_StormAttack()
{
}

EBTNodeResult::Type UBTTask_StormAttack::ExecuteTask(UBehaviorTreeComponent& OwnerComp, uint8* NodeMemory)
{
	...
	FGameplayAbilitySpec* SkillGASpec = Monster->GetAbilitySystemComponent()->FindAbilitySpecFromClass(USZGA_BansheeStormSpawn::StaticClass());
...
void UBTTask_StormAttack::OnStormSkillFinished(bool bSuccess)
{
	if (!CachedOwnerComp)
	{
		return;
