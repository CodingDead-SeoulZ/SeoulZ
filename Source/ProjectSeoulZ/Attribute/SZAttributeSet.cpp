...
#include "Monster/SZGEMonsterInit.h"
...
void USZAttributeSet::PostGameplayEffectExecute(const FGameplayEffectModCallbackData& Data)
{
    ...
    const USZGEMonsterInit* SZGE =
        Cast<USZGEMonsterInit>(Data.EffectSpec.Def);

    if (SZGE && SZGE->Tag == FGameplayTag::RequestGameplayTag(TEXT("Effect.Init.Health")))
    {
        return;
    }
