...
AActor* USZPoolManager::GetActor(TSubclassOf<AActor> ActorClass, const FTransform& SpawnTransform)
{
	...
	//		if (!IsValid(Actor))
	//		{
	//			UE_LOG(LogTemp, Error, TEXT("Pool returned invalid Actor"));
	//			continue;
	//		}
	...
	for (AActor* Actor : Pool.Actors)
	{
		...
		// 2. Pool 비활성 상태 체크 (Hidden만 쓰지 말 것)
		if (!Actor->IsHidden())
		{
			continue;
		}

		// 3. RootComponent 필수 체크
		if (!Actor->GetRootComponent())
		{
			UE_LOG(LogTemp, Error, TEXT("Pooled Actor has no RootComponent"));
			continue;
		}
