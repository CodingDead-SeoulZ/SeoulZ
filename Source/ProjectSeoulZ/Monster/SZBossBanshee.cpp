...
ASZBossBanshee::ASZBossBanshee()
{
	...
	if (RoarAnimMontageRef.Object)
	{
		...
		UE_LOG(LogTemp, Log, TEXT("RoarAnimation is nullptr"));
	}
...
void ASZBossBanshee::BeginPlay()
{
	...
	WeaponMesh->AttachToComponent(GetMesh(), FAttachmentTransformRules::SnapToTargetNotIncludingScale, "WeaponSocket");
...
{
	ASZBossAIController* BossController = Cast<ASZBossAIController>(GetController());
	UBehaviorTree* BT = BossController->GetBT();

	
	if (BossController)
	{
		BossController->RunBehaviorTree(BT);
	}
	else {
		UE_LOG(LogTemp, Log, TEXT("BT is nullptr"));
	}
