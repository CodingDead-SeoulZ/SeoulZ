...
ASZNormalMonster::ASZNormalMonster()
{
	...
	// 애님 인스턴스 할당
	static ConstructorHelpers::FClassFinder<UAnimInstance> AnimInstanceClassRef(TEXT("/Game/Animation/Monster/ABP_NormalMonster.ABP_NormalMonster_C"));
	...
	static ConstructorHelpers::FObjectFinder<USoundBase> HitSoundRef(TEXT("/Game/Sound/Monster/UndeadHit.UndeadHit"));
...
void ASZNormalMonster::HitByPlayer(const FGameplayEffectContextHandle& EffectContext)
{
	...
	APawn* UndeadPawn = Cast<APawn>(this);
	if (UndeadPawn)
	{
		if (ACharacter* MonsterCharacter = Cast<ACharacter>(UndeadPawn))
		{
			UCharacterMovementComponent* MoveComp = MonsterCharacter->GetCharacterMovement();
			if (MoveComp)
			{
				UE_LOG(LogTemp, Log, TEXT("Hit Change Speed"));
				MoveComp->MaxWalkSpeed = 500.f; // ��: �ӵ� ����
			}
		}
	}
