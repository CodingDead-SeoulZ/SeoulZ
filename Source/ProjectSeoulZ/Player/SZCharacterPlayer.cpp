...
void ASZCharacterPlayer::SetDead()
{
	...
	if (UCharacterMovementComponent* Move = GetCharacterMovement())
	{
		...
		Move->DisableMovement();  // MOVE_None 보다 명확
	}
	...
	//{
	//	if (ASZPlayerController* SZPlayerController = Cast<ASZPlayerController>(PC))
	//	{
	//		SZPlayerController->ShowMissionFailMenu();
	//	}
	//}
...
void ASZCharacterPlayer::Roll(const FInputActionValue& Value)
{
	...
	// ===== 1) 구르기 방향으로 Yaw 스냅(몽타주 재생 전에!) =====
	const FRotator RollRot = GetRollTargetRotation();
	SetActorRotation(RollRot);

	// ===== 2) 구르는 동안 회전 간섭 잠금(원복을 위해 저장) =====
	bPrevUseControllerRotationYaw = bUseControllerRotationYaw;
	bPrevOrientRotationToMovement = Move->bOrientRotationToMovement;
	bPrevUseControllerDesiredRotation = Move->bUseControllerDesiredRotation;

	bUseControllerRotationYaw = false;
	Move->bOrientRotationToMovement = false;
	Move->bUseControllerDesiredRotation = false;

	// 필요하면 (루트모션 롤이 아니라면) 롤 중 입력이 섞이지 않게 잠깐 제어:
	// Move->DisableMovement();  // 루트모션이면 케이스 따라 비추. 일단은 위 회전 잠금만 추천.
...
void ASZCharacterPlayer::OnRollMontageEnded(UAnimMontage* Montage, bool bInterrupted)
{
	...
	if (Move)
	{
		...
		Move->bUseControllerDesiredRotation = bPrevUseControllerDesiredRotation;
	}
	bUseControllerRotationYaw = bPrevUseControllerRotationYaw;

	// DisableMovement 등을 했다면 여기서 다시 Enable:
	// if (Move) Move->SetMovementMode(MOVE_Walking);
}

FRotator ASZCharacterPlayer::GetRollTargetRotation() const
{
	// 1) 입력 의도 기반
	FVector Dir = GetLastMovementInputVector();

	// 너무 작으면(입력 없거나 타이밍) 2) 속도 기반으로 대체
	...
	// Pitch/Roll은 고정하고 Yaw만 사용
	return FRotator(0.f, Target.Yaw, 0.f);
}
