...
void ASZPlayerController::Resume()
{
	// 1) UI 제거
	if (PauseMenuWidget)
	{
...
{
	// 1) Pause/MissionFail UI 모두 닫기
	if (MissionFailMenuWidget && MissionFailMenuWidget->IsInViewport())
	{
		MissionFailMenuWidget->RemoveFromParent();
	}
	if (PauseMenuWidget && PauseMenuWidget->IsInViewport())
	{
		PauseMenuWidget->RemoveFromParent();
	...
	// 3) (선택) Enhanced Input 키 상태/매핑 리빌드
	if (UEnhancedInputLocalPlayerSubsystem* Subsystem =
		ULocalPlayer::GetSubsystem<UEnhancedInputLocalPlayerSubsystem>(GetLocalPlayer()))
	{
		Subsystem->RequestRebuildControlMappings(FModifyContextOptions(), EInputMappingRebuildType::Rebuild);
	...
	if (USZGameInstance* GI = Cast<USZGameInstance>(UGameplayStatics::GetGameInstance(this)))
	{
		GI->ResetPhaseState();
	}
}

void ASZPlayerController::ExitGame()
{
	UKismetSystemLibrary::QuitGame(this, this, EQuitPreference::Quit, false);
}

void ASZPlayerController::RetryPhase1()
{
	HideMissionFailMenu();
	SetPause(false);
	ApplyGameInputMode();

	ASZGameModeBase* GameMode = Cast<ASZGameModeBase>(UGameplayStatics::GetGameMode(this));
	if (!GameMode) return;

	// 1) 플레이어가 죽어있으면 리스폰 (새 Pawn 또는 재시작)
	if (ASZCharacterPlayer* P = Cast<ASZCharacterPlayer>(GetPawn()))
	{
		if (P->IsDead())   // bIsDead getter를 하나 만들어두는 걸 권장
		{
			// 기존 Pawn 정리(선택)
			P->Destroy();

			// GameMode가 지정된 PlayerStart에 다시 스폰
			GameMode->RestartPlayer(this);
		}
		else
		{
			// 죽지 않았으면 최소한 입력 복구(혹시 막혀있을 수 있으니)
			SetIgnoreMoveInput(false);
			SetIgnoreLookInput(false);
		}
	}
	...
	// 2) Phase1 다시 시작
	GameMode->StartPhase1();
}

void ASZPlayerController::ShowBossHP(ASZBossBanshee* Boss)
{
	if (!Boss || !BossHPWidgetClass) return;
	UE_LOG(LogTemp, Log, TEXT("ShowBossHp"));
	BossHPWidget =
		CreateWidget<USZHpBarUserWidget>(this, BossHPWidgetClass);
	if (BossHPWidget)
	{
		BossHPWidget->SetAbilitySystemComponent(Boss);
...
void ASZPlayerController::HideBossHP()
{
	if (BossHPWidget)
	{
...
void ASZPlayerController::ShowMissionFailMenu()
{
	if (!MissionFailMenuClass)
	{
		UE_LOG(LogTemp, Warning, TEXT("[PC] MissionFailMenuClass is NULL"));
		return;
	}

	MissionFailMenuWidget = CreateWidget<UUserWidget>(this, MissionFailMenuClass);
	if (MissionFailMenuWidget)
	{
		MissionFailMenuWidget->AddToViewport(500);
		ApplyUIOnlyMode(MissionFailMenuWidget);
	}
	SetPause(true);
}
...
void ASZPlayerController::HideMissionFailMenu()
{
	...
	{
		MissionFailMenuWidget->RemoveFromParent();
		MissionFailMenuWidget = nullptr; // 다음에 새로 생성
	}
...
void ASZPlayerController::ShowMissionClearMenu()
{
	if (!MissionClearMenuClass)
	{
		UE_LOG(LogTemp, Warning, TEXT("[PC] MissionClearMenuClass is NULL"));
		return;
	}

	MissionClearMenuWidget = CreateWidget<UUserWidget>(this, MissionClearMenuClass);
	if (MissionClearMenuWidget)
	{
		MissionClearMenuWidget->AddToViewport(500);
		ApplyUIOnlyMode(MissionClearMenuWidget);
	}
	SetPause(true);
}

void ASZPlayerController::HideMissionClearMenu()
{
	if (MissionClearMenuWidget)
	{
		MissionClearMenuWidget->RemoveFromParent();
		MissionClearMenuWidget = nullptr; // 다음에 새로 생성
	}
