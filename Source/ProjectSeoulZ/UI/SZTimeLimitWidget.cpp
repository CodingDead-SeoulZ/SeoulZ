...
void ASZTimeLimitWidget::Tick(float DeltaSeconds)
{
	...
	if (bTimerRunning)
	{
		...
		if (RemainingTime <= WarningSeconds)
		{
	...
	if (RemainingTime > 0.0f)
	{
		...
		if (RemainingTime <= 0.0f)
		{
			...
			if (!bFailNotified)
			{
				...
				if (ASZGameModeBase* GameMode = Cast<ASZGameModeBase>(UGameplayStatics::GetGameMode(this)))
				{
					GameMode->OnPhase1TimeExpired();
				}
			}
		}
	}
...
void ASZTimeLimitWidget::StartTimer(float InTotalSeconds, float InWarningSeconds)
{
	RemainingTime = FMath::Max(0.0f, InTotalSeconds);
	WarningSeconds = FMath::Max(0.0f, InWarningSeconds);
...
void ASZTimeLimitWidget::StopTimer()
{
	bTimerRunning = false;
}